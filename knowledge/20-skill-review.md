...
- `--peer [journal]` → **Full peer review** (editor desk review → referee route → editorial decision)
...
Route the **editor** agent with the paper and target journal.
...
Route the **editor** agent again with both referee reports.
...
**Step 2: Judgment review** — route **coder-critic** in standalone mode.
...
**Do NOT edit any source files.** Only produce reports. Fixes are applied after user review, either manually or by re-routing the Coder agent.

Save report to `quality_reports/[file]_code_review.md`

### Causal Audit (`--methods`)

Route **strategist-critic** standalone for a full 4-phase causal inference review.
...
Route **writer-critic** standalone:
